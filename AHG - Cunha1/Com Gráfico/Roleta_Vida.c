...
void imprime_tudo_TESTE(populacCao* amostra){
    int i;
    printf(CINZA"\n   +----------------------- CICLO -----------------------+\n");
    printf("\t[DEBUG] Amostra: ai "AZUL"%d"CINZA", aio "AZUL"%d"CINZA", at "AZUL"%d"CINZA", soma "AZUL"%lf"CINZA"\n", amostra->qtde_de_individuo, amostra->qtde_de_individuo_ocupado, amostra->qtde_de_tarefas, amostra->soma_dos_fitness);
    for(i = 0; i < amostra->qtde_de_individuo_ocupado; i++){
        printf(CINZA"\tporcentagem "AMARELO"%lf"CINZA" | fitness "VERDE"%d "CINZA, amostra->individuo[i]->porcentagem, amostra->individuo[i]->fitness);
