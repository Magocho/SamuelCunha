...
/*
    ...
    A função mutação foi elaborada para ser colocada isoladamente no loop do AHG
        (No nosso caso optamos por 0,1% de chance de ela ocorrer)
...
void imprime_individo_TESTE(individuo* teste, int numero_de_tarefa){
    ...
    for(i = 0; i < numero_de_tarefa; i++){
        if(i == 0)
            printf("%d", teste->genes[i]);
        else printf(", %d", teste->genes[i]);
    }
    printf(" ]\n");
}
